...
namespace MySocketServerEmulator
{
    public class RequestBase
    {
        ...
        public RequestBase(XDocument doc)
        {
            XElement transactionElement = doc.Element("TRANSACTION");

            FunctionType = transactionElement.Element("FUNCTION_TYPE").Value;
            Command = transactionElement.Element("COMMAND").Value;

            if (transactionElement.Element("MAC_LABEL") != null)
                MacLabel = transactionElement.Element("MAC_LABEL").Value;
            if (transactionElement.Element("MAC") != null)
                Mac = transactionElement.Element("MAC").Value;
            if (transactionElement.Element("COUNTER") != null)
                Counter = int.Parse(transactionElement.Element("COUNTER").Value);
        }
